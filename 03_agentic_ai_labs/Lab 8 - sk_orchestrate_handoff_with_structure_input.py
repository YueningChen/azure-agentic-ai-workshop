...
from semantic_kernel.agents import (Agent, AzureAIAgent, AzureAIAgentSettings,
                                    AzureAIAgentThread, ChatCompletionAgent,
...
async def get_agents(client) -> tuple[list[Agent], OrchestrationHandoffs]:
    ...
    triage_agent = AzureAIAgent(
            ...
            plugins=[GithubPlugin()],  # Add the plugin to the agent
    )

    python_definition = await client.agents.create_agent(
        model=ai_agent_settings.model_deployment_name,
        name="PythonAgent",
    ...
    dotnet_definition = await client.agents.create_agent(
        model=ai_agent_settings.model_deployment_name,
        name="DotNetAgent",
